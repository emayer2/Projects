...
# Holds all the forces, positions, and velocities of the
# Solar System as numpy arrays for fast calculations.
# (eliminates loops)
ssPForces = []
ssMForces = []
ssPPos = []
...
for p in range(0, len(ssPlanets)):
    ...
    ssPPos = ssPPos + [ssPlanets[p].pos]
    ssPVel = ssPVel + [ssPlanets[p].velocity]
for m in range(0, len(ssMoons)):
    ssMForces = ssMForces + [vector(0, 0, 0)]
    ssMPos = ssMPos + [ssMoons[m].pos]
    ssMVel = ssMVel + [ssMoons[m].velocity]

# Converts all data to numpy arrays for fast calculations
ssPForces = array(ssPForces)
ssMForces = array(ssMForces)
ssPPos = array(ssPPos)
ssMPos = array(ssMPos)
ssPVel = array(ssPVel)
ssMVel = array(ssMVel)
ssPMasses = array(ssMasses)
ssMMasses = array(ssMoonMasses)
ssPMasses.shape = (len(ssPlanets), 1)
ssMMasses.shape = (len(ssMoons), 1)
...
# List of dropbox choices for the Solar System
ssPlanetNames = ['Sun', 'Mercury', 'Venus', 'Earth', 'Mars',
                 'Jupiter', 'Saturn', 'Uranus', 'Neptune', 'Pluto']
ssMoonNames = ['Moon', 'Deimos', 'Phobos', 'Io', 'Europa', 'Ganymede', 'Callisto']

# Solar System dropbox menus
ssLabel = wx.StaticText(p, label = 'Solar System:',
                        pos = (BUF, BUF),
                        size = wx.DefaultSize)
ssPChoices = wx.ComboBox(p, value = 'SS Planets',
                         pos = (BUF * 3.0/2.0,
                                (BUF * 3.0/2.0) + (ssLabel.Size)[1]),
                         size = wx.DefaultSize,
                         choices = ssPlanetNames)
ssMChoices = wx.ComboBox(p, value = 'SS Moons',
                         pos = ((2 * BUF) + (ssPChoices.Size)[0],
                                (BUF * 3.0/2.0) + (ssLabel.Size)[1]),
                         size = wx.DefaultSize,
                         choices = ssMoonNames)
...
sceneToggle = 0 # 0 if on planet, 1 if on moon
dt = 0          # Small span of time delta t (sec)
prevdt = dt     # Holds previous dt for time pausing
fps = 30        # Simulation runtime speed
curTime = 0     # Time per simulation frame loop (excluding rate() wait time)
totTime = 0     # Total time for the last fps number of frames
while true:
    rate(fps)  # Determines the fps speed of the simulation
    spinTime = dt
    curTime = time.time()
  
    # Calculates each respective distances between each planet/moon with
    # each other planet/moon.
    ssPR = ssPPos - ssPPos[:, newaxis]
    ssMR = ssMPos - ssMPos[:, newaxis]
    ssPMR = ssMPos - ssPPos[:, newaxis]
    ssMPR = ssPPos - ssMPos[:, newaxis]
    
    ssPRMag = sqrt(sum(square(ssPR), -1))
    ssMRMag = sqrt(sum(square(ssMR), -1))
    ssPMRMag = sqrt(sum(square(ssPMR), -1))
    ssMPRMag = sqrt(sum(square(ssMPR), -1))
    # Prevents a divide by zero warning (self-to-self p/m pair) when calculating forces.
    # Note: The distance that the self-pairs are set to
    # doesn't matter since the self forces will be eventually set to zero.
    for p in range(0, len(ssPlanets)):
        ssPRMag[p, p] = 1
    for m in range(0, len(ssMoons)):
        ssMRMag[m, m] = 1

    ssPForces = gravConst * ssPMasses * ssPMasses[:, newaxis] * ssPR / ssPRMag[:, :, newaxis]**3
    ssMForces = gravConst * ssMMasses * ssMMasses[:, newaxis] * ssMR / ssMRMag[:, :, newaxis]**3
    ssPMForces = gravConst * ssMMasses * ssPMasses[:, newaxis] * ssPMR / ssPMRMag[:, :, newaxis]**3
    ssMPForces = gravConst * ssPMasses * ssMMasses[:, newaxis] * ssMPR / ssMPRMag[:, :, newaxis]**3
    for p in range(0, len(ssPlanets)):
        ssPForces[p, p] = 0  # No self forces
    for m in range(0, len(ssMoons)):
        ssMForces[m, m] = 0  # No self forces
    
    ssPVel += dt*sum(ssPForces, 1) / ssPMasses
    ssMVel += dt*sum(ssMForces, 1) / ssMMasses
    ssPVel += dt*sum(ssPMForces,1) / ssPMasses
    ssMVel += dt*sum(ssMPForces,1) / ssMMasses
    ...
    for p in range(0, len(ssPlanets)):
        ssPlanets[p].pos = ssPPos[p]
        ssPlanets[p].velocity = ssPVel[p]
        ssPlanets[p].rotate(angle = ssSpins[p] * spinTime,
                            axis = norm(ssPlanets[p].velocity),
                            origin = ssPlanets[p].pos)
    for m in range(0, len(ssMoons)):
        ssMoons[m].pos = ssMPos[m]
        ssMoons[m].velocity = ssMVel[m]
    ...
    if scene.kb.keys:
        ...
        if k == ' ':
            ...
            if dt == 0:
                ...
                prevdt = dt
                dt = 0
    zoomControls = [ssPChoices, ssMChoices][sceneToggle]
    zoomChoices = [ssPlanets, ssMoons][sceneToggle]
    zoomPlanet = zoomControls.GetCurrentSelection()
    if zoomPlanet < 0:
        zoomPlanet = 0
    scene.center = zoomChoices[zoomPlanet].pos  # Zooms to correct planet/moon

    # Prints the debug stats if chosen to
    if frameShow:
        frameCount += 1
        totTime += time.time() - curTime
        if frameCount > fps:
            tickString =  'Average time per frame for last ' + str(fps) + ' frame(s): '
            waitString =  'Average wait time allotted per frame: '
            waitString += (' ' * abs(len(tickString) - len(waitString))) + str((1.0 / fps) - (totTime / frameCount)) + ' (sec)'
            tickString += str(totTime / frameCount) + ' (sec)'
            print(tickString)
            print(waitString)
            print('')
            frameCount = 0
...
# OLD SIMULATION LOOP CALCULATIONS
